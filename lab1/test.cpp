...
TEST(RNATest, operatorSum) {
	...
	rna2 = rna2 + A + G;
	EXPECT_TRUE(rna2.size() == size2 + 2);
	EXPECT_TRUE(rna2.capacity() >= rna2.size());
	for (unsigned int i = 0; i != size2; ++i)
		EXPECT_TRUE(rna2[i] == C);
	EXPECT_TRUE(rna2[size2] == A);
	EXPECT_TRUE(rna2[size2 + 1] == G);
...
/*TEST(DNATest, constructor) {
	...
	HuckDNA dna0(rna0, rna1);
	HuckDNA dna1(rna0 + T, rna1);
	HuckDNA dna2(rna0, rna1 + A);
	HuckDNA huckDna0 =
}*/
